...
int main(int argc, char** argv) {
    ...
    double tCompute = 0.0;
    double initMax = 0.0;
    double computeMax = 0.0;
    double overheadMax = 0.0;
    ...
    tInit = std::chrono::duration<double>(initEnd - initStart).count();
    ...
    tCompute =
    std::chrono::duration<double>(computeEnd - computeStart).count();
    ...
    MPI_Reduce(&tInit, &initMax, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&tCompute, &computeMax, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    if (rank == 0) {
    overheadMax = executionTime - initMax - computeMax;
    }
    ...
    if (rank == 0) {
        ...
        std::cout << "Initialization            : " << initMax << " s (" << initMax / executionTime * 100.0 << "%)\n";
        std::cout << "Parallel Computation      : " << computeMax << " s (" << computeMax / executionTime * 100.0 << "%)\n";
        std::cout << "MPI Overhead              : " << overheadMax << " s (" << overheadMax / executionTime * 100.0 << "%)\n";
        std::cout << "------------------------------------\n";
        std::cout << "Total                     : " << executionTime << " s\n";
    }
