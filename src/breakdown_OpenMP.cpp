...
int main(int argc, char** argv) {
    ...
    double tInit = 0.0;
    double tCompute = 0.0;          // pure Monte Carlo computation
    double tParallel = 0.0;         // entire OpenMP parallel region
    double tOverhead = 0.0;         // OpenMP overhead
                         ...
                         reduction(min:minPrice) reduction(max:maxPrice)
    {
        ...
        double localComputeStart = omp_get_wtime();
        ...
        double localComputeEnd = omp_get_wtime();
        double localCompute = localComputeEnd - localComputeStart;

        #pragma omp critical
        {
            if (localCompute > tCompute)
                tCompute = localCompute;
        }
    }

    double computeEnd = omp_get_wtime();
    tParallel = computeEnd - computeStart;
    tOverhead = tParallel - tCompute;
    ...
    std::cout << "Initialization            : " << tInit << " s (" << tInit / totalTime * 100.0 << "%)\n";
    std::cout << "Parallel Computation      : " << tCompute << " s (" << tCompute / totalTime * 100.0 << "%)\n";
    std::cout << "OpenMP Overhead           : " << tOverhead << " s (" << tOverhead / totalTime * 100.0 << "%)\n";
    std::cout << "------------------------------------\n";
